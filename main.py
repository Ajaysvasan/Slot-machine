    ...
    while True:
        ...
        if lines.isdigit():
            ...
            if 1 <= lines <= MAX_LINES:
                ...
                print("No lines must be greater than 0.")
        else:
            print("Please enter a number.")
    return lines 
...
def spin(balance):
    lines = get_no_of_line()
    while True:
        bet = get_bet()
        total_bet = bet * lines
        if total_bet > balance:
            print(f"You do not have sufficient amount for betting. Your current balance is ₹{balance}")
        else:
            break
...
def main():
    ...
    print("You left with ₹{}".format(balance))
